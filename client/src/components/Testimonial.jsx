...
const Testimonial = () => {
  return (
    <div>
        <h1 className='text-center text-2xl md:text-3xl lg:text-4xl mt-4 font-semibold bg-gradient-to-r from-zinc-900 to-zinc-400 bg-clip-text text-transparent py-5'>Customer Testimonials</h1>
       ...
       {testimonialsData.map((item,index) => (
        <div className='bg-white rounded-xl p-6 drop-shadow-md max-w-lg m-auto hover:scale-105 transition-all duration-700' key={index}>
          ...
          <div className='flex items-center gap-3 mt-5 '>
            ...
            <div>
                <p>{item.author}</p>
                <p className='text-sm text-zinc-600'>{item.jobTitle}</p>
            </div>
          </div>
        </div>
       ))}
